...
from cwjetbrains.database.base.mysql_db import MysqlDatabase
...
class HistoryDatabase(MysqlDatabase):
    ...
    def save_history(self, item):
        ...
        if count:
            logging.debug("JetBrainsjHistory already exist!")
        else:
            self._insert_for_history(item)
            logging.debug("JetBrainsHistory added to database successfully!")
