...
class DBUtils(object):
    ...
    def check_exist(self, _href):
        cursor = self.db[self.collection_name].find({'guid': self._get_guid(_href)})
        if cursor.count():
            return True

        return False

    def _get_guid(self, _href):
        """Generates an unique identifier for a given item."""
        # hash based solely in the url field
        return md5(_href).hexdigest()
