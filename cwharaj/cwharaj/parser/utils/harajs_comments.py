...
class HarajsComments(object):
    ...
    def save_for_harajs(self, hxs):
        ...
        for _comment_div in _comments_div:
            _memberName = self.baseParser.get_value_from_response(_comments_selector + '/div/div/a/text()')
            _content = self.baseParser.get_value_from_response(_comments_selector + '/div[@class="comment_body]')
